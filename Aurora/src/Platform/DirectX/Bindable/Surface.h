...
public:
	...
	void PutPixel(unsigned int x, unsigned int y, Color c) noexcept(!AU_DEBUG);
	Color GetPixel(unsigned int x, unsigned int y) const noexcept(!AU_DEBUG);
	...
	void Copy(const Surface& src) noexcept(!AU_DEBUG);
private:
