...
namespace Aurora {
	...
	void Surface::PutPixel(unsigned int x, unsigned int y, Color c) noexcept(!AU_DEBUG)
	{
		assert(x >= 0);
		assert(y >= 0);
		assert(x < width);
	...
	Surface::Color Surface::GetPixel(unsigned int x, unsigned int y) const noexcept(!AU_DEBUG)
	{
		assert(x >= 0);
		assert(y >= 0);
		assert(x < width);
	...
	void Surface::Copy(const Surface& src) noexcept(!AU_DEBUG)
	{
	...
	const char* Surface::Exception::what() const noexcept
	{
		...
		oss << AuroraException::what() << std::endl
			<< "[Note] " << GetNote();
		whatBuffer = oss.str();
